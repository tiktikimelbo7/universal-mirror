    ...
    with download_dict_lock:
            ...
            if INDEX > COUNT:
                ...
                if download.status() not in [
                    ...
                    if download.status() == MirrorStatus.STATUS_DOWNLOADING:
                        ...
                        msg += f"\n<b>♻️ Uploaded :</b> {get_readable_file_size(download.processed_bytes())}<b>\n<b>⚙️ Engine: Rclone</b>\n💾 Size</b>: {download.size()}"
                    else:
                        msg += f"\n<b>📤 Downloaded :</b> {get_readable_file_size(download.processed_bytes())}<b>\n<b>⚙️ Engine: Rclone</b>\n💾 Size</b>: {download.size()}"
                    ...
                    try:
                        ...
                        pass
                    try:
                        msg += f"\n<b>🌱 Seeder :</b> <code>{download.aria_download().num_seeders}</code>" \
                                f"\n<b>✳️ Pears :</b> <code>{download.aria_download().connections}</code>"
                    except:
                        pass
                    try:
        ...
        if STATUS_LIMIT is not None:
            ...
            if dick_no > STATUS_LIMIT:
                msg += f"<b>📑 Page :</b> <code>{PAGE_NO}/{pages}</code> | <b>📝 ᴛᴀꜱᴋꜱ :</b> <code>{dick_no}</code>\n"
